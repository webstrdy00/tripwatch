...
import { flightSearchRequestSchema, type FlightSearchInput } from "@/lib/validation/flight-schema";
...
function buildFallbackOfficialUrl(body: unknown): string {
  ...
  const query = [stringValue(record.from), "to", stringValue(record.to), stringValue(record.date)]
...
export async function POST(request: Request) {
  ...
  const parsed = flightSearchRequestSchema.safeParse(body);
  ...
  const response = await searchFlights(input as FlightSearchInput);
