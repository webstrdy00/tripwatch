...
import { intercityBusSearchRequestSchema, type BusSearchInput } from "@/lib/validation/bus-schema";
...
export async function POST(request: Request) {
  ...
  const parsed = intercityBusSearchRequestSchema.safeParse(body);
  ...
  const response = await searchIntercityBuses(input as BusSearchInput);
