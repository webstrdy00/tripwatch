...
import type { TicketScheduleData } from "@/lib/normalize/normalize-ticket";
...
function httpStatusForResponse<T>(response: TripWatchApiResponse<T>): number {
  ...
  if (response.error?.code === "NO_SCHEDULE") {
    return 404;
  }
...
async function storeResponse(response: TripWatchApiResponse<TicketScheduleData>, watchItemId: string | null) {
...
function validationFailedResponse(error: unknown, body: unknown): TripWatchApiResponse<TicketScheduleData> {
  const apiError = toApiError(error);

  return failedResponse<TicketScheduleData>({
...
export async function POST(request: Request) {
  ...
  const parsed = ticketScheduleRequestSchema.safeParse(body);
